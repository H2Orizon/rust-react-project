...
use crate::{auth::jws::create_token, models::user_model::{ActiveModel, Column, CreateUser, Entity, LoginUserForm, UserDto}};
...
pub async fn login(db: &DatabaseConnection, form:LoginUserForm) -> Result<String, DbErr>{
    ...
    let token = create_token(user.id, &env::var("SECRET_KEY").unwrap());
    Ok(token)
